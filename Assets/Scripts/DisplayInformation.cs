...
public class DisplayInformation : MonoBehaviour
{
    ...
    public void DisplayDecisionPopup(List<Decision> decisionList)
    {
        ...
        foreach (var choice in decision.availableChoices)
        {
            ...
            button.GetComponent<Button>().onClick.AddListener(delegate { ApplyChanges(choice, true); });
        }
    }
    ...
    public void CloseAllPopups()
    {
        ...
        foreach (var item in GameObject.FindGameObjectsWithTag("PopupUI")) item.SetActive(false);
        TimerScript.timerController.Pause(false);
    }

    public void ResetAvatarLocation()
    {
        foreach (var location in GameManager.instance.locationsList) location.ShowAvatar(false);
    }
    ...
    public void DisplayLocationPopup(Location location)
    {
        ...
        if (time > currentProfile.timeToSleep)
        {
            ...
            AlertDialog.instance.ShowAlert(
                $"You're out beyond your bed time ({gameTime.CalculateTimeString(currentProfile.timeToSleep)}), return home to sleep",
        ...
        if (!location.locationInformation.locationName.Equals(GameManager.instance.currentLocation.locationInformation.locationName))
        {
            ...
            var queryForLocationPopup = currentProfile.situationsForProfile.Where(x =>
                timePassedForDay >= x.startTimeToOccur
        ...
        //Detect Closing time:
        if (!(time >= locationInformation.openingTime && time <= locationInformation.closingTime) &&
            !locationInformation.is24Hours)
        {
            AlertDialog.instance.ShowAlert(
                $"{locationInformation.locationName} is closed. Opening Hours: {gameTime.CalculateTimeString(locationInformation.openingTime)} - {gameTime.CalculateTimeString(locationInformation.closingTime)}"
        ...
        GoalManager.instance.AddStat(new Stat(Stat.StatType.PlaceVisit, gameTime.ReturnDayNumber(),
            gameTime.ReturnTimePassedForDay(), 1, locationInformation.locationName, locationInformation.locationName));
        DisplayPopup(placePopup);
        LeanTween.scale(placePopup, new Vector2(1, 1), 0.25f).setFrom(new Vector2(0.5f, 0.5f))
    ...
    private void GenerateButton(FlexibleLayoutGroup layout, Choices choice)
    {
        ...
        button.GetComponent<Button>().onClick.AddListener(delegate { ApplyChanges(choice, false); });
    ...
    private string EndOfDaySummary()
    {
        ...
        var totalGoals =
            GameManager.instance.currentProfile.goals.Count(x => x.goalType == GoalManager.GoalLength.Daily);
        ...
        if (completedDaily.Count != 0)
        {
            ...
            foreach (var completeGoal in completedDaily) sb.AppendLine($"- {completeGoal.goalName}");
        }

        if (incompletedDaily.Count != 0)
        {
            sb.AppendLine("Incompleted Goals: ");
            foreach (var incompleteGoal in incompletedDaily) sb.AppendLine($"- {incompleteGoal.goalName}");
        }
